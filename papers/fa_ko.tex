...
\fidHalfScale{}을 쓰고 차단 반경은 \fidMainThreeZero{}를 쓴다. 차단 반경이 두 축에서
동시에 우위다.
...
어댑터 폭을 네 배로 늘려도 그보다 훨씬 적게 얻는다. 학습 파라미터는 \ourParams{}개로 ControlNet의
...
구별할 수 없다. 용량은 어텐션 단독 체제에서만 변화시켰고 skip이 켜진 상태에서 스템 폭을
바꾼 행이 없다. 따라서 이 표는 두 주입 설계에 대한 진술을 지지하며, 위치가 용량보다
낫다는 일반적 순서를 지지하지 않는다.}
\label{tab:where}
\end{table}
...
(ControlNet-HED는 \baseHedClip{}). 우리 조건과 데이터로 ControlNet을 학습시킨 것이 대응
통제이며, 그것은 수행하지 않았다.
